...
(function () {
  // Número de WhatsApp por defecto (editable desde Tweaks → Contacto).
  window.__WA = window.__WA || '56912345678';
  ...
  /* ---- Soundwaves: generar barras animadas ---- */
  document.querySelectorAll('.soundwave').forEach(function (wave) {
    ...
    for (var i = 0; i < bars; i++) {
      ...
      // Duración y fase variadas para un movimiento orgánico tipo ecualizador.
  ...
  /* ---- Formulario de postulación → WhatsApp ---- */
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      var url = num
        ...
        : 'https://instagram.com/Pitchmarketingchile';
...
/* ===== Animaciones de scroll (parallax, progreso, logo, contadores) ===== */
(function () {
  ...
  function onBrand() {
    ...
    var scale = (0.82 + prog * 0.18).toFixed(3);
    bbLogo.style.transform = 'scale(' + scale + ')';
  ...
  if (statsSection) {
    ...
    var cio = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting && !counted) { counted = true; runCounts(); }
      });
    }, { threshold: 0.4 });
